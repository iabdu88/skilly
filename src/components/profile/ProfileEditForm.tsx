    ...
    <form onSubmit={handleSave} className="space-y-5">
      <div className="flex items-center gap-4">
        <div className="relative">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} disabled={isUploading}
            className="absolute -bottom-1 -end-1 w-7 h-7 bg-primary rounded-full flex items-center justify-center hover:bg-primary/90 transition-colors disabled:opacity-50"
          >
            {isUploading
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{t("tapToChange")}</p>
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        <label className="text-sm font-medium text-foreground" htmlFor="bio">
          {t("bio")} <span className="text-muted-foreground font-normal">({t("optional")})</span>
        </label>
